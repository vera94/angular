...
export class LandmarkServiceService {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (isEdit) {
        that.http.put(that.addTypeUrl, data, {
          headers: new HttpHeaders().set('Authorization', that.cookieService.get("jwt")).set('Content-Type', 'application/json')
        }).subscribe((data: any) => {
          if (data) {
            resolve();
          }
  ...
  findRoute(requestDto: RequestDto) {
    var that = this;
    return new Promise(function(resolve, reject) {
      that.http.post(that.getDirectionsUrl, requestDto, {
        headers: new HttpHeaders().set('Authorization', that.cookieService.get("jwt"))
      })
      .subscribe((result: any) => {
        if (result) {
  ...
  saveSearch(requestDto: RequestDto) {
    var that = this;
    return new Promise(function(resolve, reject) {
      that.http.post(that.saveSearchUrl, requestDto, {
        headers: new HttpHeaders().set('Authorization', that.cookieService.get("jwt"))
      })
      .subscribe((result: any) => {
        resolve(result);
